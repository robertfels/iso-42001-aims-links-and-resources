# ISO 42001 & AI Governance – Curated Resources

A practical, vendor-agnostic collection of links to help teams design, implement, and operate an **AI Management System (AIMS)** aligned with **ISO/IEC 42001** and the **EU AI Act**.

> **Disclaimer:** This repository is for information only and **not legal advice**. Always consult your legal/compliance team for decisions.

---

## Who is this for?

- Compliance, Risk, Security, and Data Protection teams  
- AI/ML product owners and engineering leads  
- Internal auditors and leadership driving AI governance initiatives

---

## Contents

- [Standards & Terminology](#standards--terminology)
- [Certification Bodies](#certification-bodies)
- [EU AI Act & Regulation](#eu-ai-act--regulation)
- [Risk Management & Assessment](#risk-management--assessment)
- [Ethics & Governance](#ethics--governance)
- [Incidents & Security](#incidents--security)
- [AI Tools & Automation](#ai-tools--automation)
- [Training & Education](#training--education)
- [Economics & Strategy](#economics--strategy)
- [How to Contribute](#how-to-contribute)
- [License](#license)

---

## Standards & Terminology

### Related and Complementary Standards
- **ISO/IEC 42006** — Requirements for certification bodies
- **ISO/IEC 42005** — AI impact assessment  
- **ISO/IEC 38507** — Governance of IT (guidance for AI)
- **ISO/IEC 23894** — AI risk management
- **ISO/IEC DIS 27090** — Security of AI systems

### ISO Standards Resources
- **ISO Online Browsing Platform (OBP)**  
  Official ISO terminology and standards browser  
  https://www.iso.org/obp

- **IEC Electropedia**  
  International Electrotechnical Vocabulary  
  https://www.electropedia.org/

- **AI Standards Hub**  
  Central resource for AI-related standards and frameworks  
  https://aistandardshub.org/site-search/?_sf_s=42001

### Standards Analysis
- **ISO 42001 vs 27001 vs 9001**  
  Analysis of overlaps and differences between management standards  
  https://elevateconsult.com/insights/how-iso-42001-overlaps-with-iso-27001-and-iso-9001/

---

## Certification Bodies

- **DEKRA — ISO 42001 Certification**  
  ISO 42001 AI management system certification services  
  https://www.dekra.com/en/iso-42001-ai-management-systems/

- **SGS — ISO/IEC 42001 Certification**  
  Independent third-party certification for AI management systems  
  https://www.sgs.com/en-us/services/iso-iec-42001-certification-artificial-intelligence-ai-management-system

- **TÜV NORD — ISO 42001**  
  Certification and auditing services for AI management  
  https://www.tuv-nord.com/us/en/information-security/iso-42001-build-trust-in-responsible-ai/

- **TÜV SÜD — ISO/IEC 42001**  
  Auditing and certification for AI management systems  
  https://www.tuvsud.com/en/services/auditing-and-system-certification/iso-iec-42001

---

## EU AI Act & Regulation

### Official EU Resources
- **EU AI Act — Official Text** `(DE)`  
  Official publication of the EU Artificial Intelligence Act  
  https://eur-lex.europa.eu/legal-content/DE/TXT/HTML/?uri=OJ:L_202401689#cpt_I

- **European AI Office**  
  Information about the EU's AI Office and its role  
  https://digital-strategy.ec.europa.eu/en/policies/ai-office

- **EC Guidance — Prohibited AI Practices** `(DE)`  
  Commission guidelines on prohibited AI practices under the AI Act  
  https://digital-strategy.ec.europa.eu/de/library/commission-publishes-guidelines-prohibited-artificial-intelligence-ai-practices-defined-ai-act

### Implementation Guides
- **RTR AI Service Center** `(DE)`  
  Austrian regulatory guidance: high-level overview and provider/operator obligations  
  https://www.rtr.at/rtr/service/ki-servicestelle/ai-act/AI_Act.de.html  
  https://www.rtr.at/rtr/service/ki-servicestelle/ai-act/Betreiberverpflichtungen.de.html

- **Bitkom Implementation Guide** `(DE)` (PDF)  
  Practical implementation guide for the EU AI Act  
  https://www.bitkom.org/sites/main/files/2024-10/241028-bitkom-umsetzungsleitfaden-ki.pdf

- **CNIL AI How-to Sheets** `(FR)`  
  French data protection authority's practical AI guidance  
  https://www.cnil.fr/fr/ai-how-to-sheets

- **Bird & Bird — EU AI Act Guide** (PDF)  
  Legal guide to the European Union AI Act  
  https://www.twobirds.com/-/media/new-website-content/pdfs/capabilities/artificial-intelligence/european-union-artificial-intelligence-act-guide.pdf

### Global Regulation Tracking
- **IAPP Global AI Law & Policy Tracker** (PDF)  
  Comprehensive tracking of AI regulations worldwide  
  https://iapp.org/media/pdf/resource_center/global_ai_law_policy_tracker.pdf

- **Global AI Regulation Tracker**  
  Interactive tracker of AI regulations across countries  
  https://www.techieray.com/GlobalAIRegulationTracker

- **AI Regulations Globally — Same Goal, Different Paths**  
  Analysis of international AI regulation approaches  
  https://www.linkedin.com/pulse/ai-regulations-globally-same-goal-different-paths-dr-irina-steenbeek-1cipe/

---

## Risk Management & Assessment

### Risk Databases & Tools
- **MIT AI Risk Repository**  
  Comprehensive database of AI risks and failure modes  
  https://airisk.mit.edu/

- **AI Risk Assessment Platform**  
  Tools and frameworks for AI risk assessment  
  https://airiskassess.com/

- **UK AI Management Essentials** (PDF)  
  Self-assessment tool for AI management systems  
  https://assets.publishing.service.gov.uk/media/672a5706094e4e60c466d19f/AI_Management_Essentials_tool_Self-Assessment.pdf

---

## Ethics & Governance

### Principles & Frameworks
- **OECD AI Principles**  
  International principles for responsible AI development  
  https://oecd.ai/en/ai-principles

- **EU Ethics Guidelines for Trustworthy AI**  
  EU High-Level Expert Group guidelines on trustworthy AI  
  https://digital-strategy.ec.europa.eu/en/library/ethics-guidelines-trustworthy-ai

- **US AI Bill of Rights**  
  White House framework for AI rights and protections  
  https://bidenwhitehouse.archives.gov/ostp/ai-bill-of-rights/

### Governance Frameworks & Reports
- **Microsoft Responsible AI Standard** (PDF)  
  Microsoft's framework for responsible AI development  
  https://msblogs.thesourcemediaassets.com/sites/5/2022/06/Microsoft-Responsible-AI-Standard-v2-General-Requirements-3.pdf

- **IAPP — AI Governance in Practice Report** (PDF)  
  Industry survey and best practices for AI governance  
  https://iapp.org/media/pdf/resource_center/ai_governance_in_practice_report_2024.pdf

- **FPF — Generative AI Considerations** (PDF)  
  Future of Privacy Forum guidance on generative AI  
  https://fpf.org/wp-content/uploads/2024/06/Generative-AI-Considerations-June-24.pdf

- **CIPL — Building Accountable AI Programs** (PDF)  
  Centre for Information Policy Leadership framework  
  https://www.informationpolicycentre.com/uploads/5/7/1/0/57104281/cipl_building_accountable_ai_programs_23_feb_2024.pdf

- **ISACA Store — AI Governance Resources**  
  Professional resources and publications on AI governance  
  https://store.isaca.org/s/store#/store/browse/detail/a2S4w000007kB9pEAE

### Data Management
- **CNIL — Collecting & Qualifying Training Data**  
  Guidance on training data management and quality  
  https://www.cnil.fr/en/collecting-and-qualifying-training-data

---

## Incidents & Security

### Incident Databases
- **AI Incident Database**  
  Global database of AI-related incidents and harms  
  https://incidentdatabase.ai/

- **OECD AI Incidents Monitor**  
  Tracking and analysis of AI incidents worldwide  
  https://oecd.ai/en/incidents

### Security Frameworks
- **MITRE ATLAS**  
  Adversarial Threat Landscape for AI Systems  
  https://atlas.mitre.org/

---

## AI Tools & Automation

- **CrewAI Review** `(DE)`  
  Analysis of CrewAI multi-agent orchestration framework  
  https://textcortex.com/de/post/crewai-review

- **Make.com**  
  No-code automation and integration platform  
  https://make.com

- **n8n**  
  Open-source workflow automation tool  
  https://n8n.io

---

## Training & Education

- **mITSM — ICO Online Exam** `(DE)`  
  Online certification exam platform for ISO 42001  
  https://www.mitsm.de/ico-online

- **mITSM AI Competency Training** `(DE)`  
  ISO 42001 and AI governance training programs  
  https://mitsm.de/schulung/ki-kompetenz

---

## Economics & Strategy

- **McKinsey — The Economic Potential of Generative AI**  
  Research on AI's economic impact and productivity gains  
  https://www.mckinsey.com/capabilities/mckinsey-digital/our-insights/the-economic-potential-of-generative-ai-the-next-productivity-frontier

---

## How to Contribute

We welcome contributions!  
- **Add a resource:** Open a PR that edits `README.md` and place the link in the most relevant section with a one-line description.  
- **Quality bar:** Prefer primary sources (official standards, regulators, SDOs, or major institutions). Avoid duplicates, paywalls (when possible), and low-signal content.  
- **Issue labels:** Use `area:eu-ai-act`, `area:iso-42001`, `area:risk`, `area:security`, `area:ops`, etc.  
- **Structure:** If a section grows too large, propose splitting into its own markdown file (e.g., `eu-ai-act.md`, `risk.md`) and link it from this README.

> Tip: If you add national-language resources, include `(DE)`, `(EN)`, `(FR)`, etc. after the title.

---

## License

- **MIT**

See [`LICENSE`](./LICENSE) for details.

